...
test('첫 시작 전반/후반 기준은 단일 포인터가 아니어도 후반자부터 이어진다', () => {
  const state = stateWith({
    selectedYear: 2026,
    selectedMonthIndex: 5,
    cTeamExcludeMode: 'none',
  ...
  assert.deepEqual(getAssignment('2026-06-02'), ['선우', '찬우']);
  assert.deepEqual(getAssignment('2026-06-03'), ['찬우', '이진']);
});

test('주간 첫 시작 기준도 다음 근무일 전반은 직전 후반자부터 이어진다', () => {
  const state = stateWith({
    selectedYear: 2026,
    selectedMonthIndex: 5,
    cTeamExcludeMode: 'none',
  ...
  assert.deepEqual(getAssignment('2026-06-09'), ['서용', '민혁']);
  assert.deepEqual(getAssignment('2026-06-10'), ['민혁', '선우']);
});

test('첫 시작 이후 야간 C조는 후반자부터 이어가되 C조 인원은 건너뛴다', () => {
  ...
  assert.deepEqual(getAssignment('2026-06-01'), ['우용', '선우']);
  assert.deepEqual(getAssignment('2026-06-02'), ['찬우', '이진']);
  assert.deepEqual(getAssignment('2026-06-03'), ['이진', '윤수']);
...
test('사진 프리셋이 있어도 시작 기준 이후에는 후반자부터 이어진다', () => {
  const state = mergeState({
    version: 2,
    selectedYear: 2026,
    selectedMonthIndex: 5,
...
test('날짜별 수동 전반/후반 변경도 다음 근무일은 직전 후반자부터 이어진다', () => {
  const state = stateWith({
    selectedYear: 2026,
    selectedMonthIndex: 5,
    cTeamExcludeMode: 'none',
  ...
  assert.deepEqual(getAssignment('2026-06-08'), ['동인', '서용']);
  assert.deepEqual(getAssignment('2026-06-09'), ['서용', '민혁']);
  assert.deepEqual(getAssignment('2026-06-22'), ['영빈', '성운']);
  assert.deepEqual(getAssignment('2026-06-23'), ['성운', '승현']);
});
...
test('휴무일 비생산특근은 근무일로 바꾸지 않고 맨 위 특근 라벨로 표시된다', () => {
  const state = mergeState({
    version: 2,
    selectedYear: 2026,
    selectedMonthIndex: 5,
    overrides: {
      '2026-06-07': {
        specialWorkLabel: '비생산특근',
      },
    },
  });
  const day = generateMonthSchedule(state, 2026, 5).days.find(
    (item) => item.dateKey === '2026-06-07',
  );

  assert.equal(day?.isOff, true);
  assert.equal(day?.am, 'OFF');
  assert.equal(day?.pm, 'OFF');
  assert.equal(day?.specialWorkLabel, '비생산특근');
  assert.equal(day?.comment, '비생산특근');
});
...
test('2026년 6월부터 12월까지 C조 자동 순환과 토요일 생산특근 토글이 유지된다', () => {
  const state = mergeState({
    version: 2,
    selectedYear: 2026,
    selectedMonthIndex: 5,
  });
  const expectedCTeamKeys = new Map([
    ['2026-06', 'A'],
    ['2026-07', 'B'],
    ['2026-08', 'C'],
    ['2026-09', 'D'],
  ...
  for (let monthIndex = 5; monthIndex <= 11; monthIndex += 1) {
    const monthKey = toMonthKey(2026, monthIndex);
    assert.equal(getMonthCTeamKey(state, 2026, monthIndex), expectedCTeamKeys.get(monthKey));

    const saturday = generateMonthSchedule(state, 2026, monthIndex).days.find(
      (day) => day.dayOfWeek === 6,
    );
    assert.ok(saturday);
    assert.equal(saturday?.isOff, true);

    const overtimeOnState = mergeState({
      ...state,
      selectedMonthIndex: monthIndex,
      saturdayOvertime: {
        ...state.saturdayOvertime,
        [saturday.dateKey]: true,
      },
      overrides: {
        ...state.overrides,
        [saturday.dateKey]: {
          ...state.overrides[saturday.dateKey],
          isSaturdayOvertime: true,
          isOff: false,
        },
      },
    });
    const overtimeDay = generateMonthSchedule(overtimeOnState, 2026, monthIndex).days.find(
      (day) => day.dateKey === saturday.dateKey,
    );

    assert.equal(overtimeDay?.isOff, false);
    assert.equal(overtimeDay?.isSaturdayOvertime, true);
    assert.equal(overtimeDay?.specialWorkLabel, '생산특근');
  }
});
